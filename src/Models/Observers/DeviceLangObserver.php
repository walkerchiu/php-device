...
namespace WalkerChiu\Device\Models\Observers;

class DeviceLangObserver
{
    /**
    ...
    {
        $query =
            config('wk-core.class.device.deviceLang')
                ::withTrashed()
        ...
        if (
            config('wk-device.soft_delete')
            && (
                config('wk-core.lang_log')
                || config('wk-device.lang_log')
            )
        ) {
